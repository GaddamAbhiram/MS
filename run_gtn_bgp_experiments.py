#!/usr/bin/env python3
"""
GTN on BGP Dataset - 10 Random Seed Experiments
...
def run_experiment(seed, run_number, total_runs, gpu=0, verbose=True):
    ...
    if verbose:
        print(f"\n{'='*80}")
        print(f"[{run_number}/{total_runs}] Running GTN on BGP with seed={seed}")
        print(f"{'='*80}")
...
def main():
    ...
    print("\n" + "="*80)
    print("GTN on BGP Dataset - 10 Random Seed Experiments")
    print("="*80)
    print(f"Model: GTN (Graph Transformer Network)")
    ...
    if len(accuracies) > 0:
        ...
        output_file = f"gtn_bgp_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
