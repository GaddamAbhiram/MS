"""
ASFN Ablation Studies — Novel Component Analysis
Runs each ablation for 50 epochs on Amazon dataset and reports final test results.
"""
import subprocess
import sys
import re
...
ABLATIONS = [
    ...
    ("w/o Topology Encodings",
     ["--smooth-lambda", "0.05", "--no-topo"]),

    ("w/o Smooth Decay (decay=0)",
     ["--smooth-lambda", "0.05", "--smooth-decay", "0.0"]),

    ("w/o Global-Local Fusion",
     ["--smooth-lambda", "0.05", "--no-global-fusion"]),

    ("w/ Smooth Decay 0.75",
     ["--smooth-lambda", "0.05", "--smooth-decay", "0.75"]),
]
...
def main():
    ...
    print("  Dataset: Amazon  |  Epochs: 50  |  Device: CPU")
    print("=" * 65)
    ...
    out_file = f"ablation_results_{timestamp}.json"
