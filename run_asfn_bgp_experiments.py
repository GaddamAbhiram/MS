#!/usr/bin/env python3
"""
ASFN on BGP Dataset - 10 Random Seed Experiments
...
# Optimal hyperparameters (best ASFN config from Amazon + BGP-specific flags)
OPTIMAL_PARAMS = {
    "num_hidden": 32,
    "num_layers": 2,
    "num_heads": 1,
...
def build_command(seed, gpu=0):
    ...
    cmd = [
        ...
        "--num-heads", str(OPTIMAL_PARAMS["num_heads"]),
        "--lr", str(OPTIMAL_PARAMS["lr"]),
        "--in-drop", str(OPTIMAL_PARAMS["in_drop"]),
        "--attn-drop", str(OPTIMAL_PARAMS["attn_drop"]),
...
def run_experiment(seed, run_number, total_runs, gpu=0, verbose=True):
    ...
    if verbose:
        print(f"\n{'='*80}")
        print(f"[{run_number}/{total_runs}] Running ASFN on BGP with seed={seed}")
        print(f"{'='*80}")
...
def main():
    ...
    print("\n" + "="*80)
    print("ASFN on BGP Dataset - 10 Random Seed Experiments")
    print("="*80)
    print(f"Model: ASFN (Adaptive Smooth Fusion Network)")
    ...
    if len(accuracies) > 0:
        ...
        output_file = f"asfn_bgp_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
