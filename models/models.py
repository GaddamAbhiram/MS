...
class GraphTopoAttention(nn.Module):
    def __init__(self,
        ...
        if (self.use_fusion or self.use_adaptive_smooth) and topo_dim != in_dim:
...
class AdaptiveSmoothFusionNet(nn.Module):
    def __init__(
        ...
        self.no_topo = no_topo
        self.no_global_fusion = no_global_fusion
    ...
    def forward(self, inputs, topo):
        ...
        if self.no_global_fusion:
            fused = h
        elif self.highfreq_global:
